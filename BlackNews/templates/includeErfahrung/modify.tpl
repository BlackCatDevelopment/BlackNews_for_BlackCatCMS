...
<div id="blackNews_{$section_id}">
	...
	<div class="blackNews fc_br_all">
		<form action="{$CAT_URL}/modules/blackNews/save.php" id="bc_Options_{$section_id}" method="post" data-bnid="">
			...
			<label class="bcLabel" for="title_{$section_id}">Titel:</label>
			...
			<label class="bcLabel" for="sub_title_{$section_id}">Untertitel:</label>
			<input id="sub_title_{$section_id}" name="sub_title" type="text" class="cc_In300px" value="{if $options.sub_title}{$options.sub_title}{/if}"><br><br>

			<h3>News einbinden</h3>
			<label class="bcLabel" for="newsCount_{$section_id}">Anzahl der einzubindenden News:</label>
			<input id="newsCount_{$section_id}" name="newsCount" type="number" value="{if $options.newsCount}{$options.newsCount}{/if}"><br><br>
			<label class="bcLabel" for="setNews_{$section_id}">Sektion der Newsseite:</label>
			<select name="setNews" id="setNews_{$section_id}">
				<option value="0"{if $options.setNews == 0}selected="selected"{/if}>Keine Sektion gewählt</option>
				{foreach $sections ind s}
				<option value="{$ind}" {if $options.setNews == $ind}selected="selected"{/if}>{$s.name} (Seite: {$s.menu_title}/ Sektions-ID: {$ind})</option>
				{/foreach}
			</select>
			<br><br>
			<label class="bcLabel" for="newsCategory_{$section_id}">Kategorie (optional):</label>
